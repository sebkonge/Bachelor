...
float PID_regulator(DataVal &dataVal,
    ...
    // Convert output to range 1000ms to 2000ms (0-100% throttle)
    output = (output / 100) * (MAX_PULSE - MIN_PULSE) + MIN_PULSE;
...
void integrate_to_vel_pos(  Accel &accel, 
    ...
    float alpha = 0.98; // How much weight to give, causing the filter to be more or less responsive to the gyro
